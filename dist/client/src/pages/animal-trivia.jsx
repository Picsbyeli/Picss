...
export default function AnimalTrivia() {
    ...
    const getPerformance = () => {
        ...
        if (percentage >= 70)
            return { text: "Great job!", color: "text-blue-600" };
        if (percentage >= 50)
    ...
    return (<div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      {gameState === 'menu' && (<div className="max-w-2xl mx-auto">
          <Card>
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {['easy', 'medium', 'hard', 'mixed'].map((difficulty) => (<Button key={difficulty} variant={selectedDifficulty === difficulty ? "default" : "outline"} onClick={() => setSelectedDifficulty(difficulty)} className="h-16 text-center">
                    <div>
      ...
      {gameState === 'playing' && currentQuestion && (<div className="max-w-3xl mx-auto space-y-6">
          {/* Progress and Timer */}
              ...
              {!showAnswer ? (<div className="space-y-3">
                  <div className="grid gap-3">
                    {currentQuestion.options.map((option, index) => (<Button key={index} variant={selectedAnswer === index ? "default" : "outline"} onClick={() => handleAnswerSelect(index)} disabled={timeLeft === 0} className={`p-4 h-auto text-left justify-start transition-all ${selectedAnswer === index
                        ...
                        : 'hover:bg-green-50 hover:border-green-200'}`} data-testid={`option-${index}`}>
          ...
          {/* Action Buttons */}
          {!showAnswer && (<div className="flex gap-4">
              <Button onClick={handleSubmitAnswer} disabled={selectedAnswer === null} className="w-full py-3 text-lg" data-testid="button-submit">
                Submit Answer
              </Button>
            </div>)}
      ...
      {gameState === 'results' && (<div className="max-w-2xl mx-auto">
          <Card>
