...
import { Zap, Leaf, Flame, Droplets, Star, Heart, Sword, Shield, MapPin } from 'lucide-react';
import { Link } from 'wouter';
import { useToast } from '@/hooks/use-toast';
// Classic Pokemon-style sound effects
const playClassicSound = (soundType) => {
    ...
    console.log(`🔊 ${soundDescriptions[soundType] || '♪ Sound! ♪'}`);
};
...
export default function BurblemonPage() {
    ...
    const { data: playerBurblemons = [], isLoading: burblemonsLoading } = useQuery({
    ...
    const triggerWildEncounter = (zone) => {
        playClassicSound('select');
    ...
    return (<div className="container mx-auto px-4 py-8" data-testid="burblemon-page">
      <div className="mb-8">
      ...
      {showProfessorIntro && (<div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" data-testid="professor-intro">
          <Card className="max-w-2xl mx-4">
            ...
            <CardContent>
              ...
              <div className="flex justify-between">
                {introStep > 0 && (<Button variant="outline" onClick={() => setIntroStep(introStep - 1)}>
                    Back
                  </Button>)}
                <div className="flex-1"/>
                {introStep < professorDialogue.length - 1 ? (<Button onClick={() => setIntroStep(introStep + 1)}>
                    Continue ▶
                  </Button>) : (<Button onClick={() => setShowProfessorIntro(false)}>
                    Begin Adventure!
                  </Button>)}
              </div>
            </CardContent>
      ...
      {selectedStarter && (<div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" data-testid="starter-confirmation">
      ...
      {showWildEncounter && encounterPokemon && (<div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" data-testid="wild-encounter">
          <Card className="max-w-md mx-4">
            <CardHeader className="text-center">
      ...
      {!hasStarted ? (<Card className="max-w-4xl mx-auto" data-testid="starter-selection">
          <CardHeader className="text-center">
            <CardTitle className="text-3xl">
              Professor Bubble's Laboratory
              <Button variant="link" onClick={() => setShowProfessorIntro(true)} className="ml-2 text-blue-600 hover:text-blue-700">
                📖 Hear Introduction
              </Button>
            </CardTitle>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {starterSpecies.map((starter) => (<Card key={starter.id} className="hover:shadow-lg transition-shadow cursor-pointer border-2 hover:border-primary" onClick={() => handleStarterSelection(starter)} data-testid={`starter-${starter.name.toLowerCase()}`}>
            ...
            {chooseStarterMutation.isPending && (<div className="text-center mt-6">
                <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary mx-auto mb-2"></div>
                <p className="text-sm text-muted-foreground">Choosing your starter...</p>
              </div>)}
          </CardContent>
        </Card>) : (<Tabs value={selectedTab} onValueChange={setSelectedTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-4 bg-blue-100 dark:bg-blue-900">
            <TabsTrigger value="collection" data-testid="tab-collection" onClick={() => playClassicSound('select')} className="font-mono text-sm">
              📦 BURBLEMONS
            </TabsTrigger>
            <TabsTrigger value="map" data-testid="tab-map" onClick={() => playClassicSound('select')} className="font-mono text-sm">
              🗺️ TOWN MAP
            </TabsTrigger>
            <TabsTrigger value="inventory" data-testid="tab-inventory" onClick={() => playClassicSound('select')} className="font-mono text-sm">
              🎒 BAG
            </TabsTrigger>
            <TabsTrigger value="battles" data-testid="tab-battles" onClick={() => playClassicSound('select')} className="font-mono text-sm">
              ⚔️ TRAINER
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="collection" className="space-y-6">
            <div>
              ...
              {playerBurblemons.length === 0 ? (<Card>
                  ...
                  {playerBurblemons.map((burblemon) => {
                    ...
                    return (<Card key={burblemon.id} className="hover:shadow-lg transition-shadow" data-testid={`burblemon-${burblemon.id}`}>
                        ...
                        <CardContent className="space-y-3">
                          ...
                          {burblemon.moves.length > 0 && (<div>
                              <p className="text-xs font-medium mb-1">Moves:</p>
                              <div className="flex flex-wrap gap-1">
                                {burblemon.moves.slice(0, 4).map((move, index) => (<Badge key={index} variant="outline" className="text-xs px-1 py-0">
                                    {move}
                                  </Badge>))}
                              </div>
                            </div>)}
          ...
          <TabsContent value="map" className="space-y-6">
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {zones.map((zone) => (<Card key={zone.id} className="hover:shadow-lg transition-shadow cursor-pointer" data-testid={`zone-${zone.id}`}>
                    <CardHeader>
                    ...
                    <CardContent>
                      <div className="space-y-2 text-sm">
                        ...
                        {zone.requiredBadges > 0 && (<div className="flex justify-between text-amber-600">
                            <span>Badges Required:</span>
                            <span className="font-medium">{zone.requiredBadges}</span>
                          </div>)}
                      </div>
                      <Button className="w-full mt-4" data-testid={`explore-${zone.id}`} onClick={() => triggerWildEncounter(zone)}>
                        Explore Zone
                      </Button>
                    </CardContent>
                  </Card>))}
              ...
              <Card className="hover:shadow-lg transition-shadow">
                ...
                <CardContent className="space-y-4">
                  ...
                  <Link href="/solo-dungeon">
                    <Button className="w-full text-lg py-6" data-testid="button-enter-dungeon" onClick={() => playClassicSound('select')}>
