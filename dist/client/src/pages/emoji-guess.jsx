...
export default function EmojiGuess() {
    ...
    const [emojiType, setEmojiType] = useState('all');
    ...
    const getDerivedCategory = (puzzle) => {
        const textToCheck = (puzzle.explanation || '').toLowerCase() + ' ' + (puzzle.hint || '').toLowerCase();
        if (textToCheck.includes('movie'))
            return 'Movie';
        if (textToCheck.includes('tv') || textToCheck.includes('show') || textToCheck.includes('series'))
            return 'TV Show';
        if (textToCheck.includes('food') || textToCheck.includes('dish') || textToCheck.includes('meal'))
            return 'Food';
        if (textToCheck.includes('item') || textToCheck.includes('object') || textToCheck.includes('thing'))
            return 'Household Item';
        if (textToCheck.includes('sport') || textToCheck.includes('game') || textToCheck.includes('athletic'))
            return 'Sport/Game';
        if (textToCheck.includes('place') || textToCheck.includes('location') || textToCheck.includes('destination'))
            return 'Place';
        if (textToCheck.includes('celebrity') || textToCheck.includes('famous') || textToCheck.includes('star'))
            return 'Celebrity';
        if (textToCheck.includes('character') || textToCheck.includes('fictional'))
            return 'Character';
        if (textToCheck.includes('book') || textToCheck.includes('novel'))
            return 'Book';
        if (textToCheck.includes('song') || textToCheck.includes('music') || textToCheck.includes('band'))
            return 'Music';
        if (textToCheck.includes('car') || textToCheck.includes('vehicle') || textToCheck.includes('transport'))
            return 'Vehicle';
        if (textToCheck.includes('animal') || textToCheck.includes('creature'))
            return 'Animal';
    ...
    const { data: allPuzzles = [] } = useQuery({
        ...
        select: (data) => data.filter((puzzle) => {
            ...
            if (puzzle.category.name !== 'Emoji Guess')
                return false;
    ...
    const filteredPuzzles = allPuzzles.filter((puzzle) => puzzle.difficulty === difficulty && !solvedPuzzles.includes(puzzle.id));
    // Start a new game
    const startGame = () => {
    ...
    const checkAnswer = () => {
        ...
        let isAnswerCorrect = standardUserAnswer === standardCorrectAnswer ||
    ...
    const nextPuzzle = () => {
        ...
        const availablePuzzles = filteredPuzzles.filter((puzzle) => puzzle.id !== currentPuzzle?.id && !solvedPuzzles.includes(puzzle.id));
    ...
    return (<div className="container max-w-4xl py-8">
      ...
      {/* Game Setup */}
      {gameState === 'setup' && (<Card className="border-2 border-indigo-200 overflow-hidden shadow-md">
          ...
          <CardContent className="pt-6">
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-3 p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-200 dark:border-indigo-700 shadow-sm">
                <Badge className={`cursor-pointer px-5 py-2.5 text-base font-bold border-2 rounded-lg transition-all duration-200 transform hover:scale-105 ${difficulty === 'easy'
                ? "bg-green-600 text-white border-green-700 shadow-md dark:bg-green-500 dark:border-green-600"
                : "bg-white dark:bg-gray-800 hover:bg-green-50 dark:hover:bg-green-900/30 text-green-700 dark:text-green-400 border-green-400 dark:border-green-700"}`} onClick={() => setDifficulty('easy')}>
                  Easy (10 pts)
                </Badge>
                <Badge className={`cursor-pointer px-5 py-2.5 text-base font-bold border-2 rounded-lg transition-all duration-200 transform hover:scale-105 ${difficulty === 'medium'
                ? "bg-blue-600 text-white border-blue-700 shadow-md dark:bg-blue-500 dark:border-blue-600"
                : "bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/30 text-blue-700 dark:text-blue-400 border-blue-400 dark:border-blue-700"}`} onClick={() => setDifficulty('medium')}>
                  Medium (15 pts)
                </Badge>
                <Badge className={`cursor-pointer px-5 py-2.5 text-base font-bold border-2 rounded-lg transition-all duration-200 transform hover:scale-105 ${difficulty === 'hard'
                ? "bg-amber-600 text-white border-amber-700 shadow-md dark:bg-amber-500 dark:border-amber-600"
                : "bg-white dark:bg-gray-800 hover:bg-amber-50 dark:hover:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-amber-400 dark:border-amber-700"}`} onClick={() => setDifficulty('hard')}>
                  Hard (25 pts)
                </Badge>
                <Badge className={`cursor-pointer px-5 py-2.5 text-base font-bold border-2 rounded-lg transition-all duration-200 transform hover:scale-105 ${difficulty === 'extreme'
                ? "bg-red-600 text-white border-red-700 shadow-md dark:bg-red-500 dark:border-red-600"
                : "bg-white dark:bg-gray-800 hover:bg-red-50 dark:hover:bg-red-900/30 text-red-700 dark:text-red-400 border-red-400 dark:border-red-700"}`} onClick={() => setDifficulty('extreme')}>
                  Extreme (40 pts)
                </Badge>
              </div>
            </div>
            ...
            <div className="mb-8">
              ...
              <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                <Tabs defaultValue="all" onValueChange={(value) => setEmojiType(value)} className="w-full">
                  <TabsList className="grid grid-cols-4 mb-4 bg-white p-1.5 border-2 border-indigo-200 rounded-lg">
                    <TabsTrigger value="all" className="font-bold text-base data-[state=active]:bg-gradient-to-r data-[state=active]:from-indigo-500 data-[state=active]:to-purple-600 data-[state=active]:text-white data-[state=active]:shadow-md rounded-md px-4 py-2.5">
                      All Types
                    </TabsTrigger>
                    <TabsTrigger value="movies" className="font-bold text-base data-[state=active]:bg-gradient-to-r data-[state=active]:from-indigo-500 data-[state=active]:to-purple-600 data-[state=active]:text-white data-[state=active]:shadow-md rounded-md px-4 py-2.5">
                      Movies
                    </TabsTrigger>
                    <TabsTrigger value="foods" className="font-bold text-base data-[state=active]:bg-gradient-to-r data-[state=active]:from-indigo-500 data-[state=active]:to-purple-600 data-[state=active]:text-white data-[state=active]:shadow-md rounded-md px-4 py-2.5">
                      Foods
                    </TabsTrigger>
                    <TabsTrigger value="household-items" className="font-bold text-base data-[state=active]:bg-gradient-to-r data-[state=active]:from-indigo-500 data-[state=active]:to-purple-600 data-[state=active]:text-white data-[state=active]:shadow-md rounded-md px-4 py-2.5">
                      Items
                    </TabsTrigger>
                  </TabsList>
          ...
          <CardFooter className="bg-gradient-to-r from-indigo-50 to-purple-50 border-t-2 border-indigo-200 p-6">
            <Button onClick={startGame} className="w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold py-4 px-6 rounded-lg shadow-lg text-lg border-2 border-green-600 transition-all duration-200 transform hover:scale-105">
      ...
      {/* Game Play */}
      {gameState === 'playing' && currentPuzzle && (<div className="space-y-6">
          {/* Game Info */}
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <Badge className={`py-1 px-3 font-semibold ${difficulty === 'easy'
                ...
                : difficulty === 'medium'
                  ...
                  {currentPuzzle &&
                <FavoriteButton riddleId={currentPuzzle.id} size="sm" alwaysShow={true} className="transition-all duration-300 hover:scale-110 ml-1"/>}
                </div>
              </div>
              {currentPuzzle.explanation && (<div className="mt-3 bg-gradient-to-r from-yellow-100 to-orange-100 border-2 border-yellow-300 rounded-lg p-4 shadow-sm">
                  <div className="flex items-center gap-2">
            ...
            <CardContent className="flex flex-col items-center">
              ...
              {/* Hint */}
              {showHint && (<div className="bg-gradient-to-r from-amber-50 to-yellow-100 border-2 border-amber-300 p-4 rounded-lg mb-6 w-full shadow-md">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-end">
                      <AIHintButton riddle={`These emoji '${currentPuzzle.question}' represent '${currentPuzzle.answer}' which is a ${currentPuzzle.explanation.toLowerCase().includes('movie') ? 'movie' :
                    currentPuzzle.explanation.toLowerCase().includes('tv') ? 'TV show' :
                        currentPuzzle.explanation.toLowerCase().includes('food') ? 'food item' :
                            currentPuzzle.explanation.toLowerCase().includes('item') ? 'household item' :
                                'entertainment concept'}. Give a middle-school friendly hint that helps understand what these emoji represent.`}/>
                    </div>
                  </div>
                </div>)}
              
              {/* Answer Input */}
              {!isCorrect ? (<div className="w-full max-w-md bg-white p-4 rounded-lg border-2 border-indigo-200 shadow-md">
                  <h3 className="font-bold text-indigo-800 mb-3 text-center">Enter Your Answer</h3>
                  <div className="flex gap-2 mb-4">
                    <Input type="text" placeholder="Your answer..." value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} className="flex-1 border-2 border-indigo-300 focus:border-indigo-500 h-11"/>
                    <Button onClick={checkAnswer} disabled={!userAnswer.trim()} className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold px-5 h-11">
                      Check
                    </Button>
                  </div>
                  
                  <div className="flex gap-2">
                    <Button variant="outline" onClick={getHint} disabled={showHint} className="flex-1 border-2 border-amber-400 text-amber-700 hover:bg-amber-50 font-semibold">
                      Get Hint (-5 points)
                    </Button>
                    <Button variant="outline" className="flex-1 border-2 border-red-400 text-red-700 hover:bg-red-50 font-semibold" onClick={() => {
                  ...
                  <Button onClick={nextPuzzle} className="w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold py-3 text-lg shadow-md">
                    Next Puzzle →
                  </Button>
                </div>)}
            </CardContent>
      ...
      {/* Game Complete */}
      {gameState === 'complete' && (<Card className="border-4 border-purple-300 shadow-lg overflow-hidden">
          ...
          <CardContent className="text-center py-8 bg-gradient-to-br from-white to-purple-50">
            ...
            <Button onClick={resetGame} className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold py-3 px-8 rounded-lg shadow-lg text-lg">
              Play Again
            </Button>
          </CardContent>
        </Card>)}
