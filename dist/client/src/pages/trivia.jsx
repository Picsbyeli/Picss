...
export default function TriviaPage() {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Game Menu */}
        {gameState === 'menu' && (<div className="space-y-6">
            <Card>
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Button variant={selectedDifficulty === 'easy' ? 'default' : 'outline'} className="p-6 h-auto" onClick={() => setSelectedDifficulty('easy')}>
                  ...
                  <Button variant={selectedDifficulty === 'medium' ? 'default' : 'outline'} className="p-6 h-auto" onClick={() => setSelectedDifficulty('medium')}>
                    <div className="text-center">
                      <div className="text-2xl mb-2">🟡</div>
                  ...
                  <Button variant={selectedDifficulty === 'hard' ? 'default' : 'outline'} className="p-6 h-auto" onClick={() => setSelectedDifficulty('hard')}>
                    <div className="text-center">
                      <div className="text-2xl mb-2">🔴</div>
                  ...
                  <Button variant={selectedDifficulty === 'mixed' ? 'default' : 'outline'} className="p-6 h-auto" onClick={() => setSelectedDifficulty('mixed')}>
                    <div className="text-center">
                      <div className="text-2xl mb-2">🎲</div>
        ...
        {/* Game Playing */}
        {gameState === 'playing' && currentQuestion && (<div className="space-y-6">
            {/* Progress and Stats */}
            <Card>
              <CardContent className="pt-6">
            ...
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  <Badge className={`${currentQuestion.difficulty === 'easy' ? 'bg-green-100 text-green-800' :
              ...
              <CardContent className="space-y-4">
                {!showAnswer ? (<div className="space-y-3">
                    <div className="grid gap-3">
                      {currentQuestion.options.map((option, index) => (<Button key={index} variant={selectedAnswer === index ? "default" : "outline"} onClick={() => handleAnswerSelect(index)} disabled={timeLeft === 0} className={`p-4 h-auto text-left justify-start transition-all ${selectedAnswer === index
                        ? 'bg-indigo-600 text-white border-indigo-600'
                        : 'hover:bg-indigo-50 hover:border-indigo-200'}`} data-testid={`option-${index}`}>
                          <div className="flex items-center gap-3 w-full">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-semibold ${selectedAnswer === index
                    ...
                    <Button onClick={handleSubmitAnswer} disabled={selectedAnswer === null || timeLeft === 0} className="w-full py-3 text-lg" data-testid="button-submit">
                      Submit Answer
                    </Button>
                  </div>) : (<div className="space-y-4">
                    <div className="grid gap-3">
                      {currentQuestion.options.map((option, index) => (<div key={index} className={`p-4 rounded-lg border-2 transition-all ${index === currentQuestion.correct
                        ? 'bg-green-50 border-green-200'
                        : selectedAnswer === index
                            ? 'bg-red-50 border-red-200'
                            : 'bg-gray-50 border-gray-200'}`}>
                          <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-semibold ${index === currentQuestion.correct
                        ...
                        : selectedAnswer === index
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {selectedAnswer === index && index !== currentQuestion.correct && (<XCircle className="w-5 h-5 text-red-600"/>)}
                            </div>
                          </div>
                        </div>))}
                    </div>
                    <div className={`p-4 rounded-lg border-2 ${lastAnswerCorrect
                    ? 'bg-green-50 border-green-200'
                    : 'bg-red-50 border-red-200'}`}>
                      <div className="flex items-center gap-2 mb-2">
                        {lastAnswerCorrect ? (<CheckCircle className="w-5 h-5 text-green-600"/>) : (<XCircle className="w-5 h-5 text-red-600"/>)}
                        <span className="font-bold text-lg">
                          {lastAnswerCorrect ? 'Correct!' : 'Incorrect'}
                        </span>
                      </div>
                      <p className="text-lg"><strong>Correct Answer:</strong> {currentQuestion.options[currentQuestion.correct]}</p>
                      {selectedAnswer !== null && (<p className="text-lg"><strong>Your Answer:</strong> {currentQuestion.options[selectedAnswer]}</p>)}
                    </div>
        ...
        {/* Game Finished */}
        {gameState === 'finished' && (<div className="space-y-6">
            <Card>
              <CardHeader>
