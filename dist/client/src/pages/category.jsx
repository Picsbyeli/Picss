...
export default function CategoryPage() {
    ...
    const { riddlesWithCategories, categories, getRandomUnsolvedRiddle, getRiddlesByCategory, getUnsolvedRiddlesByCategory, isLoadingRiddles, isLoadingCategories } = useRiddles();
    const { currentRiddle, setCurrentRiddle, userAnswer, setUserAnswer, isCorrectModalOpen, setIsCorrectModalOpen, isWrongModalOpen, setIsWrongModalOpen, timeLeft, hintsUsed, pointsEarned, timeToSolve, checkAnswer, getHint, resetGame, userStats, isCheckingAnswer, isPaused, currentHint, showAnswer, attemptCount, MAX_ATTEMPTS } = useGame();
    ...
    return (<div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-br from-purple-600 via-blue-600 to-indigo-700 text-white">
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <Button variant="outline" className="text-white border-white hover:bg-white hover:text-purple-600" onClick={handleRandomRiddle}>
      ...
      {/* Current Riddle */}
      <div className="container mx-auto px-4 py-8">
        {currentRiddle ? (<div className="max-w-4xl mx-auto">
            <CurrentRiddle riddle={currentRiddle} userAnswer={userAnswer} setUserAnswer={setUserAnswer} onCheckAnswer={checkAnswer} onGetHint={getHint} onNextRiddle={handleNextRiddle} onPreviousRiddle={handlePreviousRiddle} timeLeft={timeLeft} hintsUsed={hintsUsed} isCheckingAnswer={isCheckingAnswer} isPaused={isPaused} riddlesCount={categoryRiddles.length} currentRiddleIndex={categoryRiddles.findIndex(r => r.id === currentRiddle.id) + 1} onShowAnswer={showAnswer}/>
      ...
      {/* Modals */}
      <CorrectAnswerModal isOpen={isCorrectModalOpen} onClose={() => setIsCorrectModalOpen(false)} riddleAnswer={currentRiddle?.answer || ""} explanation={currentRiddle?.explanation || ""} timeToSolve={timeToSolve} pointsEarned={pointsEarned} onNextRiddle={() => {
      ...
      <WrongAnswerModal isOpen={isWrongModalOpen} onClose={() => setIsWrongModalOpen(false)} userAnswer={userAnswer} onShowAnswer={() => {
        ...
        }} hint={currentHint || currentRiddle?.hint || "No hint available."} isAttemptsExhausted={attemptCount >= MAX_ATTEMPTS}/>
    </div>);
}
