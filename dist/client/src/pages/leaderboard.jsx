...
export default function LeaderboardPage() {
    ...
    return (<div className="container max-w-5xl px-4 md:px-6 py-6 md:py-12">
      ...
      {user && userRank && (<Card className="mb-8 border-2 border-primary/20 bg-primary/5">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <CustomAvatarDisplay config={user.avatarConfig ? JSON.parse(user.avatarConfig) : null} username={user.username} profileImageUrl={user.profileImageUrl} size={48}/>
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {leaderboard.length > 0 ? (leaderboard.map((player, index) => (<TableRow key={player.id} className={player.id === user?.id ? "bg-primary/5 font-medium" : ""}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <CustomAvatarDisplay config={player.avatarConfig ? JSON.parse(player.avatarConfig) : null} username={player.username} profileImageUrl={player.profileImageUrl} size={32}/>
                        <div className="flex flex-col">
                          <span className="font-medium">{player.username}</span>
                          {player.id === user?.id && (<Badge variant="outline" className="w-fit text-xs">You</Badge>)}
                        </div>
                      </div>
                    </TableCell>
