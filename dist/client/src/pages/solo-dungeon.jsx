...
export default function SoloDungeonPage() {
    ...
    // State
    const [selectedBurblemon, setSelectedBurblemon] = useState(null);
    ...
    const { data: playerBurblemons = [] } = useQuery({
    ...
    const submitAnswerMutation = useMutation({
        mutationFn: async ({ dungeonId, questionId, answer }) => {
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 gap-2">
                <Button onClick={handleContinueBattle} className="flex items-center gap-2" data-testid="button-continue-battle">
                  <Sword className="h-4 w-4"/>
                  Continue Fighting
                </Button>
                <Button onClick={handleExploreLoot} variant="outline" className="flex items-center gap-2" data-testid="button-explore-loot">
      ...
      <Dialog open={showLootResults} onOpenChange={setShowLootResults}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            ...
            {lootResults?.itemsFound && lootResults.itemsFound.length > 0 && (<div className="space-y-3">
                <h3 className="font-medium">Items Found:</h3>
                <div className="grid gap-2">
                  {lootResults.itemsFound.map((item, index) => (<div key={`${item.id}-${index}`} className="flex items-center justify-between p-3 rounded-lg border bg-muted/20">
            ...
            <div className="flex justify-center pt-4">
              <Button onClick={() => setShowLootResults(false)} className="w-full" data-testid="button-close-loot-results">
                Continue Adventure
              </Button>
            </div>
          </div>
      ...
      <Tabs value={battlePhase === 'select' ? 'select' : 'battle'} className="space-y-6">
        ...
        <TabsContent value="select" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            return (<Card key={burblemon.id} className={`cursor-pointer transition-colors hover:bg-accent ${selectedBurblemon?.id === burblemon.id ? 'ring-2 ring-primary' : ''}`} onClick={() => setSelectedBurblemon(burblemon)} data-testid={`burblemon-${burblemon.id}`}>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {topics.map((topic) => (<Card key={topic.id} className={`cursor-pointer transition-colors hover:bg-accent ${selectedTopic?.id === topic.id ? 'ring-2 ring-primary' : ''}`} onClick={() => setSelectedTopic(topic)} data-testid={`topic-${topic.id}`}>
                    <CardContent className="p-4">
                      <div className="flex items-center gap-3">
          ...
          <Card>
            <CardContent className="p-6 text-center">
              <Button onClick={handleStartBattle} disabled={!selectedBurblemon || !selectedTopic || startDungeonMutation.isPending} size="lg" className="text-lg px-8 py-6" data-testid="button-start-dungeon">
          ...
          {activeDungeon && selectedBurblemon && selectedSpecies && (<>
              ...
              {currentQuestion && (<Card>
                  <CardHeader>
                    ...
                    <Badge className={`w-fit ${currentQuestion.difficulty === 'easy' ? 'bg-green-100 text-green-800' :
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {currentQuestion.questionType === 'multiple_choice' && currentQuestion.choices ? (<div className="grid grid-cols-1 gap-2">
                        {JSON.parse(currentQuestion.choices).map((choice, index) => (<Button key={index} variant={currentAnswer === choice ? 'default' : 'outline'} onClick={() => setCurrentAnswer(choice)} className="justify-start" data-testid={`choice-${index}`}>
                            {choice}
                          </Button>))}
                      </div>) : (<input type="text" value={currentAnswer} onChange={(e) => setCurrentAnswer(e.target.value)} className="w-full p-2 border rounded" placeholder="Enter your answer..." data-testid="text-answer-input"/>)}

                    <Button onClick={handleSubmitAnswer} disabled={!currentAnswer.trim() || submitAnswerMutation.isPending} className="w-full" data-testid="button-submit-answer">
              ...
              {/* Get Next Question */}
              {!currentQuestion && activeDungeon.isActive && (<Card>
                  <CardContent className="p-6 text-center">
                    <Button onClick={() => nextQuestionMutation.mutate(activeDungeon.id)} disabled={nextQuestionMutation.isPending} size="lg" data-testid="button-next-question">
