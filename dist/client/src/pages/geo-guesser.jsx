...
export default function GeoGuesser() {
    ...
    const handleInteractiveMapComplete = async (results) => {
        try {
            if (usStatesMap) {
                const resultData = {
    ...
    const submitGuess = () => {
        ...
        if (gameMode === 'world') {
            ...
            const continent = currentLocation;
    ...
    return (<div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <Card className="mb-8">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {['world', 'usStates', 'continents', 'interactive'].map(mode => {
                const Icon = getModeIcon(mode);
                return (<Button key={mode} variant={gameMode === mode ? "default" : "outline"} className="h-24 flex flex-col gap-2" onClick={() => setGameMode(mode)}>
        ...
        {(gameMode === 'interactive' || gameMode === 'usStates') && !gameOver && (<div className="space-y-6">
            <Card>
              <CardHeader>
              ...
              <CardContent>
                {isLoadingMap ? (<div className="text-center py-8">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-2"></div>
                    <p>Loading interactive map...</p>
                  </div>) : usStatesMap ? (<InteractiveMap map={usStatesMap} onGameComplete={handleInteractiveMapComplete}/>) : (<div className="text-center py-8 text-muted-foreground">
        ...
        {gameActive && !gameOver && gameMode !== 'interactive' && gameMode !== 'usStates' && (<Card>
            <CardHeader>
              ...
              <div className="space-y-6">
                <div className="text-center">
                  ...
                  <div className="bg-gray-100 rounded-lg p-4 mb-4">
                    <div className="text-center">
                      {gameMode === 'world' && currentLocation && landmarkImages[currentLocation.name] && (<div>
                          <img src={landmarkImages[currentLocation.name]} alt="Famous landmark" className="w-full max-w-md mx-auto rounded-lg shadow-lg mb-3" style={{ maxHeight: '300px', objectFit: 'cover' }}/>
                          <p className="text-sm text-gray-600">📍 Where is this famous landmark located?</p>
                        </div>)}
                      {gameMode === 'world' && currentLocation && !landmarkImages[currentLocation.name] && (<div className="text-gray-500">
                      ...
                      {gameMode === 'usStates' && (<div className="text-gray-500">
                      ...
                      {gameMode === 'continents' && (<div className="text-gray-500">
                ...
                {/* Hint Section */}
                {!showResult && (<div className="border-t pt-4">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-medium text-gray-700">Need extra help?</h4>
                      {!hintUsed && (<Button variant="outline" size="sm" onClick={useHint} className="text-orange-600 border-orange-200 hover:bg-orange-50" data-testid="button-hint">
                          <HelpCircle className="w-4 h-4 mr-1"/>
                          Get Hint (-75% points)
                        </Button>)}
                      {hintUsed && (<div className="flex items-center text-orange-600 text-sm">
                          <AlertTriangle className="w-4 h-4 mr-1"/>
                          Hint Used
                        </div>)}
                    </div>
                    
                    {showDetailedHint && (<div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4">
                ...
                {!showResult && (<div className="space-y-4">
                    <div>
                      ...
                      <Input id="guess" value={guess} onChange={(e) => setGuess(e.target.value)} placeholder={`Enter ${gameMode === 'world' ? 'country, city or landmark' : gameMode === 'usStates' ? 'state name or capital' : 'continent name'}`} onKeyPress={(e) => e.key === 'Enter' && submitGuess()} data-testid="input-guess"/>
                    </div>
                ...
                {showResult && (<div className="text-center space-y-4">
