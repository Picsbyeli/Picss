...
import { Brain, Calculator, BookOpen, Lightbulb, Puzzle, Target, ChevronRight, Swords, Zap, Volume2, GraduationCap, Globe, Trophy, Gamepad2 } from "lucide-react";
import { Link } from "wouter";
export default function Home() {
    const { categories, getRiddlesByCategory, isLoadingCategories } = useRiddles();
    const { userStats } = useGame();
    // Get category icon
    const getCategoryIcon = (categoryName) => {
        const name = categoryName.toLowerCase();
        if (name.includes('math') || name.includes('number'))
            return Calculator;
        if (name.includes('logic'))
            return Brain;
        if (name.includes('word') || name.includes('riddle'))
            return BookOpen;
    ...
    const getCategoryColor = (categoryName) => {
        const name = categoryName.toLowerCase();
        if (name.includes('math') || name.includes('number'))
            return 'bg-blue-100 text-blue-800 border-2 border-blue-500 hover:bg-blue-200';
        if (name.includes('logic'))
            return 'bg-purple-100 text-purple-800 border-2 border-purple-500 hover:bg-purple-200';
        if (name.includes('word') || name.includes('riddle'))
            return 'bg-green-100 text-green-800 border-2 border-green-500 hover:bg-green-200';
        if (name.includes('puzzle'))
            return 'bg-orange-100 text-orange-800 border-2 border-orange-500 hover:bg-orange-200';
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-cyan-50">
      {/* Hero Section */}
      ...
      <div className="bg-white/80 backdrop-blur-sm shadow-xl border-t-4 border-gradient-to-r from-pink-400 to-purple-400">
        <div className="container mx-auto px-4 py-8 md:py-16">
          ...
          {/* Riddle Categories */}
          {!isLoadingCategories && categories.length > 0 && (<div>
              <div className="flex items-center justify-center gap-2 mb-8">
                <span className="text-2xl md:text-3xl">🧩</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                ...
                return (<Link key={category.id} href={`/category/${category.id}`}>
      ...
      <div className="bg-gradient-to-br from-purple-100 via-pink-50 to-indigo-100 py-12 md:py-20">
        ...
        }} onStartNewGame={() => window.location.href = `/category/${categories[0]?.id || 25}`} isLoading={false}/>
            </div>
          </div>
                ...
                <CardContent className="p-8 md:p-12 relative">
                  <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="flex-1 text-center md:text-left">
                      ...
                      <Link href="/multiplayer">
                        <Button variant="secondary" size="lg" className="bg-white text-red-600 hover:bg-yellow-100 hover:text-red-700 px-8 py-4 text-lg font-bold rounded-full shadow-lg transform hover:scale-105 transition-all duration-200">
