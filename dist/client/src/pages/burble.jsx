...
export default function Burble() {
    ...
    const getHint = () => {
        if (gameDifficulty !== 'hard' || hintUsed)
            return;
    ...
    const renderAttempts = () => {
        return attempts.map((attempt, index) => (<div key={index} className="flex items-center mb-3">
        <Badge className="mr-3 bg-primary text-white w-8 h-8 flex items-center justify-center text-sm">
          {scores[index]}
        </Badge>
        <div className="flex space-x-2">
          {attempt.split('').map((letter, letterIndex) => (<div key={letterIndex} className="w-10 h-10 border-2 border-gray-300 rounded-md flex items-center justify-center uppercase font-bold text-lg">
              {letter}
            </div>))}
        </div>
      </div>));
    ...
    const renderEmptySlots = () => {
        return Array.from({ length: parseInt(wordLength) }).map((_, index) => (<div key={index} className="w-10 h-10 border-2 border-gray-300 rounded-md flex items-center justify-center"></div>));
    };
    return (<div className="container mx-auto px-4 py-8">
      <div className="max-w-xl mx-auto">
        <div className="bg-white rounded-xl shadow-md p-6 mb-8">
          ...
          {gameState === 'setup' && (<div className="space-y-6">
              <div>
                <label className="block text-sm font-medium mb-2">Game Difficulty</label>
                <Select value={gameDifficulty} onValueChange={(value) => setGameDifficulty(value)}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Select value={wordLength} onValueChange={(value) => setWordLength(value)}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Select value={category} onValueChange={(value) => setCategory(value)}>
                  ...
                  <SelectContent>
                    {Object.keys(wordCategories).map((cat) => (<SelectItem key={cat} value={cat}>{cat}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
          ...
          {gameState === 'loading' && (<div className="flex flex-col items-center justify-center py-8">
          ...
          {(gameState === 'playing' || gameState === 'gameover') && (<div>
              ...
              <div className="space-y-6">
                ...
                {gameState === 'playing' && (<div className="space-y-4">
                    <div className="flex space-x-2">
                      {renderEmptySlots()}
                    </div>
                    
                    <div className="flex space-x-2">
                      <Input value={currentAttempt} onChange={(e) => setCurrentAttempt(e.target.value.toLowerCase())} placeholder={`Enter a ${wordLength}-letter word`} maxLength={parseInt(wordLength)} className="flex-1"/>
                      <Button onClick={submitAttempt}>Submit</Button>
                    </div>
                    
                    {/* Show hint button only in hard mode and if hint not used yet */}
                    {gameDifficulty === 'hard' && !hintUsed && (<div className="flex justify-center pt-2">
                        <Button variant="outline" onClick={getHint} className="text-sm">
                          Use Hint (1 available)
                        </Button>
                      </div>)}
                    
                    {/* Display hint message if hint was used */}
                    {showHint && (<div className="bg-yellow-50 border border-yellow-200 rounded-md p-3 text-center text-yellow-800 text-sm">
                        Hint used! Check the toast message for your hint.
                      </div>)}
                  </div>)}

                {/* Game over actions */}
                {gameState === 'gameover' && (<div className="flex justify-center space-x-4 pt-4">
