      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <Card className="mb-8">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {Object.keys(schoolTriviaQuestions).map(subject => {
                const Icon = getSubjectIcon(subject);
                return (<Button key={subject} variant={selectedSubject === subject ? "default" : "outline"} className="h-20 flex flex-col gap-2" onClick={() => setSelectedSubject(subject)}>
            ...
            <Card className="mb-8">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {['elementary', 'middle', 'high'].map(grade => (<Button key={grade} variant={selectedGrade === grade ? "default" : "outline"} className="h-16" onClick={() => setSelectedGrade(grade)}>
                      {getGradeName(grade)}
                    </Button>))}
                </div>
              </CardContent>
        ...
        {gameActive && !gameOver && (<Card>
            ...
            <CardContent>
              <div className="space-y-6">
                ...
                return (<Button key={index} variant="outline" className={buttonClass} onClick={() => selectAnswer(index)} disabled={showResult}>
                        <span className="flex items-center">
                          <span className="bg-gray-200 rounded-full w-6 h-6 flex items-center justify-center mr-3 text-sm font-bold">
                            {String.fromCharCode(65 + index)}
                          </span>
                          {option}
                          {showResult && index === currentQuestions[currentQuestion].correct && (<span className="ml-auto text-green-600">✓</span>)}
                          {showResult && index === selectedAnswer && selectedAnswer !== currentQuestions[currentQuestion].correct && (<span className="ml-auto text-red-600">✗</span>)}
                        </span>
                      </Button>);
            })}
                </div>
                ...
                <div className="flex gap-4">
                  {!showResult ? (<Button onClick={submitAnswer} disabled={selectedAnswer === null}>
                      Submit Answer
        ...
        {gameOver && (<Card>
            <CardContent className="p-8 text-center">
              ...
              <div className="mb-6">
                {correctAnswers >= totalQuestions * 0.8 ? (<div className="text-green-600">
                    <h3 className="text-xl font-bold">🎓 Excellent Work!</h3>
                    <p>Outstanding academic performance!</p>
                  </div>) : correctAnswers >= totalQuestions * 0.6 ? (<div className="text-blue-600">
