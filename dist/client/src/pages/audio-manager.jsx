...
import { Volume2, Upload, Trash2, Play, Cloud, Plus } from "lucide-react";
...
export default function AudioManager() {
    ...
    const convertToAudioTrack = (file) => {
        ...
        return {
            ...
            duration: null, // Will be determined when loaded
            fileSize: file.size,
            mimeType: file.type,
    ...
    const addToCurrentPlaylist = (file) => {
        const audioTrack = convertToAudioTrack(file);
        const mockTrack = {
            id: parseInt(file.id.replace(/\D/g, '') || '1'),
            ...audioTrack,
      ...
      <div className="bg-gradient-to-r from-pink-500 to-red-600 text-white">
        <div className="container mx-auto px-4 py-12">
          <div className="text-center">
            ...
            {firebaseUser && (<div className="flex items-center justify-center mt-4 text-sm opacity-75">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                    ...
                    <Input id="audio-upload" type="file" accept="audio/*" multiple onChange={handleFileUpload} className="mt-1"/>
                  </div>
          ...
          <TabsContent value="library" className="space-y-6">
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                {audioFiles.length > 0 && (<Button onClick={playAllFiles} className="flex items-center gap-2" data-testid="button-play-all">
                    <Play className="h-4 w-4"/>
                    Play All ({audioFiles.length})
                  </Button>)}
              </div>
              
              {audioFiles.length === 0 ? (<Card>
                  ...
                  {audioFiles.map(file => (<Card key={file.id}>
                      <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="ml-4 flex items-center gap-2">
                            <Button variant="default" size="sm" onClick={() => playTrack(file)} className="flex items-center gap-1" data-testid={`button-play-${file.id}`}>
                              <Play className="h-3 w-3"/>
                              Play
                            </Button>
                            <Button variant="outline" size="sm" onClick={() => addToCurrentPlaylist(file)} data-testid={`button-add-playlist-${file.id}`} title="Add to current playlist">
                              <Plus className="h-3 w-3"/>
                            </Button>
                            <Button variant="destructive" size="sm" onClick={() => deleteAudioFile(file.id)} data-testid={`button-delete-${file.id}`}>
