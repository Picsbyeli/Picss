...
export function USStatesMap({ selectedState, onStateClick, correctStates = new Set(), incorrectStates = new Set(), currentTargetState }) {
    const [hoveredState, setHoveredState] = useState('');
    const getStateColor = (stateId) => {
        if (correctStates.has(stateId))
            return '#10B981'; // Green for correct
        if (incorrectStates.has(stateId))
            return '#EF4444'; // Red for incorrect  
        if (currentTargetState === stateId)
            return '#F59E0B'; // Orange for target
        if (hoveredState === stateId)
            return '#6366F1'; // Blue for hover
        if (selectedState === stateId)
            return '#8B5CF6'; // Purple for selected
        return '#94A3B8'; // Default gray
    };
    const getStateStrokeColor = (stateId) => {
        if (currentTargetState === stateId)
            return '#D97706'; // Darker orange for target outline
        return '#475569'; // Default dark gray stroke
    };
    return (<div className="w-full max-w-4xl mx-auto bg-blue-50 dark:bg-blue-950 rounded-lg p-4">
      <svg viewBox="0 0 1000 600" className="w-full h-auto cursor-pointer" style={{ maxHeight: '500px' }}>
        ...
        {/* State paths */}
        {US_STATES.map(state => (<path key={state.id} d={state.path} fill={getStateColor(state.id)} stroke={getStateStrokeColor(state.id)} strokeWidth={currentTargetState === state.id ? "3" : "1.5"} className="transition-all duration-200 hover:brightness-110" onMouseEnter={() => setHoveredState(state.id)} onMouseLeave={() => setHoveredState('')} onClick={() => onStateClick(state.id, state.name)} data-testid={`state-${state.id}`}>
      ...
      {currentTargetState && (<div className="mt-4 p-3 bg-yellow-100 dark:bg-yellow-900 rounded-lg text-center">
