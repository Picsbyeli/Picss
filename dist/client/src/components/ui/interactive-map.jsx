...
export function InteractiveMap({ map, onGameComplete }) {
    const [gameStarted, setGameStarted] = useState(false);
    const [currentTargetRegion, setCurrentTargetRegion] = useState(null);
    const [correctStates, setCorrectStates] = useState(new Set());
    const [incorrectStates, setIncorrectStates] = useState(new Set());
    ...
    const handleStateClick = (stateCode, stateName) => {
        if (!currentTargetRegion || showResults || !gameStarted)
            return;
    ...
    if (!gameStarted) {
        return (<Card className="w-full max-w-2xl mx-auto" data-testid="card-geography-start">
    ...
    if (showResults) {
        return (<Card className="w-full max-w-4xl mx-auto" data-testid="card-results">
        <CardHeader className="text-center">
          <div className="flex justify-center mb-4">
        ...
        <CardContent className="space-y-6">
          ...
          <USStatesMap correctStates={correctStates} incorrectStates={incorrectStates} onStateClick={() => { }} // Disabled after game
        />
          ...
          <Button onClick={startGame} className="w-full" data-testid="button-play-again">
            Play Again
          </Button>
        </CardContent>
      </Card>);
    ...
    return (<Card className="w-full max-w-4xl mx-auto" data-testid="card-geography-game">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        {currentTargetRegion && (<div className="text-center bg-blue-50 dark:bg-blue-950 p-6 rounded-lg">
            ...
            {currentTargetRegion.capital && (<p className="text-sm text-muted-foreground mt-1">
                ({currentTargetRegion.capital} is the capital)
              </p>)}
          </div>)}

        {/* Interactive Map */}
        <USStatesMap correctStates={correctStates} incorrectStates={incorrectStates} currentTargetState={currentTargetRegion?.code} onStateClick={handleStateClick}/>

        {/* Hint Section */}
        {showHint && currentTargetRegion?.capital && (<div className="bg-yellow-100 dark:bg-yellow-900 rounded-lg p-4">
        ...
        <div className="flex gap-2 justify-center">
          <Button onClick={useHint} variant="outline" data-testid="button-use-hint">
            <Lightbulb className="mr-2 h-4 w-4"/>
            Hint
          </Button>
          <Button onClick={skipRegion} variant="outline" data-testid="button-skip">
