...
export default function AIRiddleGenerator({ trigger }) {
    ...
    return (<>
      {trigger ? (<div onClick={openDialog}>{trigger}</div>) : (<Button className="flex items-center gap-1 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white" onClick={openDialog}>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <ScrollArea className="h-[350px] border rounded-md p-4">
            {isGeneratingRiddle ? (<div className="flex flex-col items-center justify-center py-16">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500 mb-4"></div>
                <p className="text-gray-500">Generating a unique riddle...</p>
              </div>) : generatedRiddle ? (<div className="space-y-6">
                <div>
                ...
                {generatedRiddle.citations.length > 0 && (<div>
                    <h3 className="text-sm font-semibold text-gray-700">Sources:</h3>
                    <ul className="text-xs text-gray-500 space-y-1 mt-2">
                      {generatedRiddle.citations.map((citation, i) => (<li key={i}>
                          <a href={citation} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline truncate block">
                            {citation}
                          </a>
                        </li>))}
                    </ul>
                  </div>)}
              </div>) : (<div className="flex flex-col items-center justify-center h-full text-gray-400 py-16">
          ...
          <DialogFooter className="flex justify-between">
            <div className="flex gap-2">
              <Button variant="outline" onClick={handleGenerateRiddle} disabled={isGeneratingRiddle} className="flex items-center gap-1">
                <RefreshCw className={`w-4 h-4 ${isGeneratingRiddle ? 'animate-spin' : ''}`}/>
                Generate New
              </Button>
              
              <Button onClick={handleSaveRiddle} disabled={!generatedRiddle || isSaving || !selectedCategory} className="flex items-center gap-1 bg-green-600 hover:bg-green-700">
