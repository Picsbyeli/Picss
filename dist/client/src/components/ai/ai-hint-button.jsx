...
export default function AIHintButton({ riddle, onGetHint }) {
    ...
    return (<>
      <Button variant="outline" size="sm" className="flex items-center gap-1 bg-gradient-to-r from-purple-500 to-blue-400 hover:from-purple-600 hover:to-blue-500 text-white border-none" onClick={handleGetAIHint} disabled={isGeneratingHint}>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen} modal={false}>
        <DialogContent className="max-w-md" onPointerDownOutside={(e) => e.preventDefault()}>
          ...
          <ScrollArea className="max-h-[300px] mt-4">
            {isGeneratingHint ? (<div className="flex flex-col items-center justify-center py-8">
                ...
                {citations.length > 0 && (<div className="space-y-2">
                    <h4 className="text-sm font-semibold text-gray-700">Sources:</h4>
                    <ul className="text-xs text-gray-500 space-y-1">
                      {citations.map((citation, i) => (<li key={i}>
