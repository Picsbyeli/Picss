...
export const WrongAnswerModal = ({ isOpen, onClose, userAnswer, onShowAnswer, onTryAgain, onNextQuestion, hint, isAttemptsExhausted = false }) => {
      ...
      <DialogContent className="sm:max-w-md mx-4 rounded-xl shadow-xl p-6 fade-in" onPointerDownOutside={(e) => e.preventDefault()}>
        <div className="text-center">
          ...
          <div className="flex space-x-3">
            <Button onClick={onShowAnswer} variant="outline" className="flex-1 bg-light-gray hover:bg-light-gray/80 text-dark font-medium py-3 rounded-lg transition duration-200">
              <i className="ri-lightbulb-line mr-1"></i>
              Show Answer
            </Button>
            {isAttemptsExhausted && onNextQuestion ? (<Button onClick={onNextQuestion} className="flex-1 bg-green-600 hover:bg-green-700 text-white font-medium py-3 rounded-lg transition duration-200">
                <i className="ri-arrow-right-line mr-1"></i>
                Next Question
              </Button>) : (<Button onClick={onTryAgain} className="flex-1 bg-primary hover:bg-primary/90 text-white font-medium py-3 rounded-lg transition duration-200">
                Try Again
              </Button>)}
          </div>
