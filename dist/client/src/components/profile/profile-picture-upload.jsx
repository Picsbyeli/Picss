...
export function ProfilePictureUpload({ currentImageUrl, username, onUploadComplete }) {
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-col items-center space-y-4">
          ...
          <Avatar className="w-24 h-24">
            {previewUrl ? (<AvatarImage src={previewUrl} alt={username}/>) : (<AvatarFallback className="bg-gradient-to-br from-purple-500 to-pink-500 text-white text-2xl">
          ...
          <div className="flex gap-2">
            <Button onClick={triggerFileInput} disabled={isUploading || updateProfilePictureMutation.isPending} className="flex items-center gap-2">
              {isUploading || updateProfilePictureMutation.isPending ? (<Loader2 className="h-4 w-4 animate-spin"/>) : (<Upload className="h-4 w-4"/>)}
              {previewUrl ? 'Change Picture' : 'Upload Picture'}
            </Button>

            {previewUrl && (<Button variant="outline" onClick={handleRemove} disabled={isUploading || updateProfilePictureMutation.isPending} className="flex items-center gap-2">
