...
export function GameMenu() {
    ...
    return (<DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="sm" className="flex items-center gap-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white border-none hover:from-purple-600 hover:to-pink-600" data-testid="button-games-menu">
      ...
      <DropdownMenuContent align="start" className="w-64 max-h-96 overflow-y-auto">
        <div className="p-2">
          ...
          {games.map((game) => (<Link key={game.path} href={game.path}>
              <DropdownMenuItem className="flex items-start gap-3 p-3 cursor-pointer hover:bg-purple-50 rounded-md" data-testid={`menu-item-${game.path.replace('/', '') || 'home'}`}>
