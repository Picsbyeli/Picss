...
export default function SubmitRiddleForm() {
    ...
    const submitRiddleMutation = useMutation({
        mutationFn: async (values) => {
            ...
            const response = await apiRequest("POST", "/api/riddles/submit", parsedValues);
    ...
    return (<div className="bg-white rounded-xl shadow-sm p-6 mb-8">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          {/* Question */}
          <FormField control={form.control} name="question" render={({ field }) => (<FormItem>
                <FormLabel className="font-medium">Riddle Question</FormLabel>
                <FormControl>
                  <Textarea {...field} placeholder="Write your riddle question here..." className="min-h-[100px]" disabled={isSubmitting}/>
                </FormControl>
                <FormDescription>
          ...
          {/* Answer */}
          <FormField control={form.control} name="answer" render={({ field }) => (<FormItem>
                <FormLabel className="font-medium">Answer</FormLabel>
                <FormControl>
                  <Input {...field} placeholder="The solution to your riddle" disabled={isSubmitting}/>
                </FormControl>
                <FormDescription>
          ...
          {/* Hint */}
          <FormField control={form.control} name="hint" render={({ field }) => (<FormItem>
                <FormLabel className="font-medium">Hint (Optional)</FormLabel>
                <FormControl>
                  <Input {...getFieldProps(field)} placeholder="Provide a helpful hint" disabled={isSubmitting}/>
                </FormControl>
                <FormDescription>
          ...
          {/* Explanation */}
          <FormField control={form.control} name="explanation" render={({ field }) => (<FormItem>
                <FormLabel className="font-medium">Explanation (Optional)</FormLabel>
                <FormControl>
                  <Textarea {...getFieldProps(field)} placeholder="Explain the answer..." className="min-h-[80px]" disabled={isSubmitting}/>
                </FormControl>
                <FormDescription>
          ...
          <FormField control={form.control} name="imageUrl" render={({ field }) => (<FormItem>
                ...
                <FormControl>
                  <Input {...getFieldProps(field)} placeholder="https://example.com/image.jpg" type="url" disabled={isSubmitting}/>
                </FormControl>
                <FormDescription>
          ...
          {/* Creator Name */}
          <FormField control={form.control} name="creatorName" render={({ field }) => (<FormItem>
                <FormLabel className="font-medium">Creator Name</FormLabel>
                <FormControl>
                  <Input {...getFieldProps(field)} placeholder="Your name or nickname" disabled={isSubmitting}/>
                </FormControl>
                <FormDescription>
