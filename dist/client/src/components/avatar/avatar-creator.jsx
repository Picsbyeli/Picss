...
import { Shuffle, Save, User, Sparkles } from 'lucide-react';
const defaultConfig = {
    faceShape: 'round',
    skinTone: '#FDBCB4',
    hairStyle: 'medium',
...
export default function AvatarCreator({ initialConfig = defaultConfig, onSave, username }) {
    ...
    const randomizeAvatar = () => {
        const randomConfig = {
            faceShape: ['round', 'oval', 'square', 'heart'][Math.floor(Math.random() * 4)],
            skinTone: skinTones[Math.floor(Math.random() * skinTones.length)],
            hairStyle: ['short', 'medium', 'long', 'curly', 'bald', 'ponytail', 'buzz'][Math.floor(Math.random() * 7)],
            hairColor: hairColors[Math.floor(Math.random() * hairColors.length)],
            eyeStyle: ['normal', 'large', 'small', 'sleepy', 'wink'][Math.floor(Math.random() * 5)],
            eyeColor: eyeColors[Math.floor(Math.random() * eyeColors.length)],
            glasses: ['none', 'regular', 'sunglasses', 'round', 'cat-eye'][Math.floor(Math.random() * 5)],
            hat: ['none', 'cap', 'beanie', 'fedora', 'crown'][Math.floor(Math.random() * 5)],
            backgroundColor: backgroundColors[Math.floor(Math.random() * backgroundColors.length)],
            expression: ['happy', 'neutral', 'excited', 'cool', 'surprised'][Math.floor(Math.random() * 5)],
        };
    ...
    return (<Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="flex items-center gap-2" data-testid="button-open-avatar-creator">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col items-center space-y-4">
            ...
            <div className="flex gap-2">
              <Button onClick={randomizeAvatar} variant="outline" size="sm" className="flex items-center gap-2" data-testid="button-randomize-avatar">
                <Shuffle className="h-4 w-4"/>
                Randomize
              </Button>
              <Button onClick={handleSave} className="flex items-center gap-2" data-testid="button-save-avatar">
            ...
            <Tabs defaultValue="face" className="w-full">
              ...
              <TabsContent value="face" className="space-y-4">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['round', 'oval', 'square', 'heart'].map(shape => (<Button key={shape} variant={config.faceShape === shape ? "default" : "outline"} size="sm" onClick={() => updateConfig({ faceShape: shape })} data-testid={`face-shape-${shape}`}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {skinTones.map(tone => (<Button key={tone} className={`h-8 w-8 rounded-full border-2 ${config.skinTone === tone ? 'ring-2 ring-primary ring-offset-2' : ''}`} style={{ backgroundColor: tone }} onClick={() => updateConfig({ skinTone: tone })} data-testid={`skin-tone-${tone}`}/>))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['happy', 'neutral', 'excited', 'cool', 'surprised'].map(expression => (<Button key={expression} variant={config.expression === expression ? "default" : "outline"} size="sm" onClick={() => updateConfig({ expression })} data-testid={`expression-${expression}`}>
              ...
              <TabsContent value="hair" className="space-y-4">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['short', 'medium', 'long', 'curly', 'bald', 'ponytail', 'buzz'].map(style => (<Button key={style} variant={config.hairStyle === style ? "default" : "outline"} size="sm" onClick={() => updateConfig({ hairStyle: style })} data-testid={`hair-style-${style}`}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {hairColors.map(color => (<Button key={color} className={`h-8 w-8 rounded-full border-2 ${config.hairColor === color ? 'ring-2 ring-primary ring-offset-2' : ''}`} style={{ backgroundColor: color }} onClick={() => updateConfig({ hairColor: color })} data-testid={`hair-color-${color}`}/>))}
                  </div>
                </div>
              </TabsContent>
              ...
              <TabsContent value="eyes" className="space-y-4">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['normal', 'large', 'small', 'sleepy', 'wink'].map(style => (<Button key={style} variant={config.eyeStyle === style ? "default" : "outline"} size="sm" onClick={() => updateConfig({ eyeStyle: style })} data-testid={`eye-style-${style}`}>
                        {style.charAt(0).toUpperCase() + style.slice(1)}
                      </Button>))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {eyeColors.map(color => (<Button key={color} className={`h-8 w-8 rounded-full border-2 ${config.eyeColor === color ? 'ring-2 ring-primary ring-offset-2' : ''}`} style={{ backgroundColor: color }} onClick={() => updateConfig({ eyeColor: color })} data-testid={`eye-color-${color}`}/>))}
                  </div>
                </div>
              </TabsContent>
              ...
              <TabsContent value="accessories" className="space-y-4">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['none', 'regular', 'sunglasses', 'round', 'cat-eye'].map(glasses => (<Button key={glasses} variant={config.glasses === glasses ? "default" : "outline"} size="sm" onClick={() => updateConfig({ glasses })} data-testid={`glasses-${glasses}`}>
                        {glasses === 'none' ? 'None' : glasses.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                      </Button>))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['none', 'cap', 'beanie', 'fedora', 'crown'].map(hat => (<Button key={hat} variant={config.hat === hat ? "default" : "outline"} size="sm" onClick={() => updateConfig({ hat })} data-testid={`hat-${hat}`}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {backgroundColors.map(color => (<Button key={color} className={`h-8 w-8 rounded-full border-2 ${config.backgroundColor === color ? 'ring-2 ring-primary ring-offset-2' : ''}`} style={{ backgroundColor: color }} onClick={() => updateConfig({ backgroundColor: color })} data-testid={`bg-color-${color}`}/>))}
                  </div>
                </div>
              </TabsContent>
            </Tabs>
