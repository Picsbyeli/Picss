...
export default function RiddleCard({ riddle, onPlayNow }) {
    // Check user progress to see if this riddle has been solved
    const { data: userProgress = [] } = useQuery({
        queryKey: [`/api/user/1/progress`],
        enabled: !!riddle,
    });
    const isSolved = Array.isArray(userProgress) && userProgress.some((progress) => progress.riddleId === riddle.id && progress.solved);
    ...
    return (<div className="riddle-card flex flex-col bg-white rounded-xl shadow-md overflow-hidden border-2 border-transparent hover:border-primary/30 transition-all duration-300 hover:-translate-y-2">
      ...
      <div className="relative p-4">
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center gap-1">
            ...
            {isSolved ? (<span className="bg-success/10 text-success text-xs font-semibold px-3 py-1.5 rounded-full flex items-center">
                <i className="ri-check-line mr-1"></i>
                Solved!
              </span>) : (<span className="bg-gradient-to-r from-amber-100 to-amber-200 text-amber-700 px-3 py-1.5 rounded-full text-xs font-medium">
                {riddle.difficulty === 'easy' && (<span className="flex items-center">
                    <i className="ri-star-fill mr-0.5 text-amber-500"></i>
                    Easy
                  </span>)}
                {riddle.difficulty === 'medium' && (<span className="flex items-center">
                    <i className="ri-star-fill mr-0.5 text-amber-500"></i>
                    <i className="ri-star-fill mr-0.5 text-amber-500"></i>
                    Medium
                  </span>)}
                {riddle.difficulty === 'hard' && (<span className="flex items-center">
                    <i className="ri-star-fill mr-0.5 text-amber-500"></i>
                    <i className="ri-star-fill mr-0.5 text-amber-500"></i>
                    <i className="ri-star-fill mr-0.5 text-amber-500"></i>
                    Hard
        ...
        {riddle.imageUrl && (<div className="mb-3 overflow-hidden rounded-lg relative border-2 border-primary/20 shadow-md">
            <img src={riddle.imageUrl} alt="Visual riddle" className="w-full h-36 object-cover transition-transform hover:scale-105 duration-500"/>
      ...
      <div className="mt-auto bg-gradient-to-r from-gray-50 to-gray-100 px-4 py-3 flex justify-between items-center border-t border-gray-100">
        ...
        <button className="fun-button text-white text-xs font-bold py-1.5 px-4 rounded-full" onClick={onPlayNow}>
