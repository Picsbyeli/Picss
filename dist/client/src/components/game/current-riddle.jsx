...
export default function CurrentRiddle({ riddle, userAnswer, setUserAnswer, onCheckAnswer, onGetHint, onNextRiddle, onPreviousRiddle, timeLeft, // Kept for backward compatibility
hintsUsed, isCheckingAnswer, isPaused, riddlesCount, currentRiddleIndex, onShowAnswer, // New callback for showing answer
 }) {
    ...
    const getCategoryStyles = (categoryClass) => {
    ...
    return (<div className="bg-white rounded-xl shadow-sm p-4 md:p-6 mb-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center">
          ...
          <div className="flex items-center">
            {Array.from({ length: 5 }).map((_, index) => (<div key={index} className={`w-3 h-3 mx-0.5 rounded-full ${index < hintsUsed ? 'bg-accent' : 'bg-light-gray'}`}/>))}
      ...
      <div className="mb-6 text-center py-6">
        ...
        {riddle.imageUrl && (<div className="mb-6 flex justify-center">
            <div className="relative">
              <img src={riddle.imageUrl} alt="Visual riddle" className="max-w-full max-h-64 object-contain rounded-xl shadow-lg border-4 border-primary/20 transition-all duration-300 hover:scale-102"/>
        ...
        {riddle.answer === "" ? (<>
            <div className="my-8 p-8 bg-gradient-to-br from-green-50 to-teal-50 border-2 border-green-200 rounded-2xl shadow-lg">
              ...
              <button onClick={onNextRiddle} className="fun-button mt-4 px-8 py-3 bg-white text-green-700 border-2 border-green-500 rounded-full font-bold transition-all shadow-lg hover:shadow-xl hover:bg-green-50">
            ...
            {riddle.imageUrl && (<div className="bg-yellow-50 border border-yellow-200 rounded-full inline-block px-4 py-1 mx-auto">
      ...
      {riddle.answer !== "" && (<>
          ...
          {showAnswer && (<div className="mb-8 p-6 bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl border-2 border-primary/20 shadow-lg text-center relative overflow-hidden">
              {/* Decorative elements */}
              ...
              <div className="relative z-10">
                ...
                {riddle.explanation && (<div className="mt-4 pt-4 border-t border-primary/10">
                ...
                <div className="mt-6 pt-4 border-t border-primary/10">
                  <button className="bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 text-white flex items-center px-6 py-3 rounded-full font-bold shadow-lg transition transform duration-200 hover:scale-105 mx-auto" onClick={onNextRiddle}>
          ...
          <form onSubmit={handleSubmit} className="mb-8 max-w-xl mx-auto">
            ...
            <div className="flex shadow-lg rounded-full overflow-hidden border-2 border-primary/30 bg-white">
              <Input type="text" placeholder="Type your answer here..." className="flex-1 border-0 focus:ring-2 focus:ring-primary focus:ring-opacity-30 rounded-l-full px-6 py-4 text-base font-medium outline-none transition-all duration-300" value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} disabled={isCheckingAnswer || isPaused}/>
              <Button className="fun-button px-8 py-4 rounded-r-full font-bold transition-all" type="submit" disabled={isCheckingAnswer || !userAnswer.trim() || isPaused}>
                {isCheckingAnswer ? (<div className="flex items-center">
          ...
          <div className="max-w-2xl mx-auto">
            <div className="flex flex-wrap justify-center gap-4 mb-6">
              ...
              <div className="flex flex-wrap gap-3 justify-center">
                <div className="relative group">
                  <button className={`bg-gradient-to-br ${hintsUsed >= 5 ? 'from-gray-300 to-gray-400 opacity-70' : 'from-yellow-400 to-amber-500 hover:from-yellow-500 hover:to-amber-600'} text-white flex items-center px-4 py-2 rounded-xl font-bold shadow-md transition transform duration-200 ${hintsUsed < 5 ? 'hover:scale-105' : ''}`} onClick={onGetHint} disabled={isPaused || hintsUsed >= 5}>
                ...
                {/* Show Answer Button */}
                <button className={`bg-gradient-to-br ${showAnswer ? 'from-gray-300 to-gray-400 opacity-70' : 'from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700'} text-white flex items-center px-4 py-2 rounded-xl font-bold shadow-md transition transform duration-200 ${!showAnswer ? 'hover:scale-105' : ''}`} onClick={() => {
            ...
            <div className="flex justify-center gap-4 mb-4">
              <button className="bg-gradient-to-r from-blue-400 to-cyan-500 hover:from-blue-500 hover:to-cyan-600 text-white flex items-center px-5 py-2 rounded-full font-bold shadow-md transition transform duration-200 hover:scale-105" onClick={onPreviousRiddle} disabled={isPaused}>
                <i className="ri-arrow-left-line mr-1.5 text-lg"></i>
                Previous
              </button>
              <button className="bg-gradient-to-r from-teal-400 to-green-500 hover:from-teal-500 hover:to-green-600 text-white flex items-center px-5 py-2 rounded-full font-bold shadow-md transition transform duration-200 hover:scale-105" onClick={onNextRiddle} disabled={isPaused}>
                Next
      ...
      {riddle.answer === "" && (<div className="flex justify-center mt-6 mb-4">
          <button className="bg-gradient-to-r from-blue-400 to-cyan-500 hover:from-blue-500 hover:to-cyan-600 text-white flex items-center px-5 py-2 rounded-full font-bold shadow-md transition transform duration-200 hover:scale-105" onClick={onPreviousRiddle}>
            <i className="ri-arrow-left-line mr-1.5 text-lg"></i>
            Previous Puzzle
          </button>
        </div>)}
