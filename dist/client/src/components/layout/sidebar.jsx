...
export default function Sidebar({ isOpen, onClose, userStats }) {
    ...
    // Mobile sidebar
    const MobileSidebar = () => (<Sheet open={isOpen} onOpenChange={onClose}>
    ...
    const SidebarContent = ({ userStats, user }) => (<>
        ...
        <ul className="space-y-2">
          ...
          <li className="mt-4 border-t border-light-gray dark:border-gray-700 pt-4">
            <a href="/profile" className="flex items-center p-2">
              <CustomAvatarDisplay config={user?.avatarConfig ? JSON.parse(user.avatarConfig) : null} username={user?.username || 'Guest User'} profileImageUrl={user?.profileImageUrl} size={40}/>
              <div className="ml-3">
