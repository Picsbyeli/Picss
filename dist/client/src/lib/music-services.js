...
export class YouTubeService {
    ...
    async searchTracks(query, limit = 20) {
        try {
            const response = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoCategoryId=10&maxResults=${limit}&q=${encodeURIComponent(query + ' music')}&key=${this.apiKey}`);
            if (!response.ok) {
    ...
    async getTrack(id) {
        try {
            const response = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&id=${id}&key=${this.apiKey}`);
            if (!response.ok)
                return null;
            const data = await response.json();
...
export class MusicServiceManager {
    ...
    async getTrackById(platform, id) {
        switch (platform) {
            case 'spotify':
    ...
    convertToAudioTrack(externalTrack) {
        return {
