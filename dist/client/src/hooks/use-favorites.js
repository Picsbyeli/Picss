...
export function useFavorites() {
    ...
    const { data: favorites = [], isLoading: isLoadingFavorites, error: favoritesError, } = useQuery({
        queryKey: ['/api/user', user?.id, 'favorites'],
        enabled: !!user,
    });
    // Check if a riddle is favorited
    const checkIsFavorited = async (riddleId) => {
        if (!user)
            return false;
        try {
    ...
    const toggleFavorite = (riddleId, options) => {
