...
export function useRiddles() {
    ...
    const categories = useMemo(() => {
        return allCategories.filter(category => category.name.toLowerCase() !== 'fan made');
    }, [allCategories]);
    // Get all riddles with categories
    const { data: allRiddlesWithCategories = [], isLoading: isLoadingRiddles, error: riddlesError, } = useQuery({
    ...
    const getRiddlesByCategory = (categoryId) => {
        if (categoryId === null) {
    ...
    const getUnsolvedRiddlesByCategory = (categoryId) => {
        if (categoryId === null) {
    ...
    const getRandomUnsolvedRiddle = (categoryId = null) => {
