...
const generateProgressiveHints = (riddleQuestion, riddleAnswer, baseHint) => {
    const hints = [];
...
export function useGame() {
    ...
    const { data: userStats = { score: 0, solvedCount: 0, avgTimeSeconds: 0 }, refetch: refetchUserStats, } = useQuery({
    ...
    const markRiddleViewedMutation = useMutation({
        mutationFn: async () => {
            if (!currentRiddle || !user)
                return null;
            const response = await apiRequest("POST", "/api/mark-viewed", {
    ...
    const startGame = () => {
        ...
        if (currentRiddle) {
            const hints = generateProgressiveHints(currentRiddle.question, currentRiddle.answer, currentRiddle.hint);
    ...
    const getHint = () => {
        ...
        if (progressiveHints.length > 0 && hintsUsed < progressiveHints.length) {
            ...
            if (user) {
                apiRequest("GET", `/api/hints/${currentRiddle.id}?userId=${user.id}`, undefined).then(() => {
                    // Update user stats
                    refetchUserStats();
                });
            }
