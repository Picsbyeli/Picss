import { users, categories, riddles, userProgress, userFavorites, gameSessions, gameParticipants, gameAnswers, quizTopics, quizQuestions, dungeonRuns, burblemonSpecies, playerBurblemons, mapZones, zonePaths, playerMapProgress, wildEncounters, items, playerInventory, playerEconomy, battleRecords, geoMaps, geoRegions, geoGameResults } from "@shared/schema";
import { db } from "./db";
...
export class MemStorage {
    ...
    async getRiddlesWithCategories() {
        ...
        return Promise.all(riddlesArray.map(async (riddle) => {
    ...
    async createOrUpdateUserProgress(progress) {
        ...
        const existingProgress = await this.getUserRiddleProgress(progress.userId, progress.riddleId);
        if (existingProgress) {
    ...
    async updateUserStatsAfterSolve(userId, timeToSolveSeconds, riddleId) {
        ...
        const newAvgTime = Math.round((user.avgTimeSeconds * user.solvedCount + timeToSolveSeconds) / newSolvedCount);
    ...
    findFavoriteByUserAndRiddle(userId, riddleId) {
        for (const favorite of this.userFavorites.values()) {
...
export class DatabaseStorage {
    ...
    async removeUsersByEmail(emailPattern) {
        ...
        const usersToRemove = await db
            .select()
            .from(users)
            .where(sql `${users.email} ILIKE ${`%${emailPattern}%`}`);
    ...
    async getUserRiddleProgress(userId, riddleId) {
        const [progress] = await db
            .select()
            .from(userProgress)
            .where(and(eq(userProgress.userId, userId), eq(userProgress.riddleId, riddleId)));
        return progress;
    }
    async createOrUpdateUserProgress(progress) {
    ...
    async toggleUserFavorite(userId, riddleId) {
        ...
        const existing = await db.select()
            .from(userFavorites)
            .where(and(eq(userFavorites.userId, userId), eq(userFavorites.riddleId, riddleId)));
    ...
    async joinGameSession(sessionId, userId) {
        ...
        const existing = await db.select().from(gameParticipants)
            .where(and(eq(gameParticipants.sessionId, sessionId), eq(gameParticipants.userId, userId)));
        if (existing.length > 0) {
    ...
    async processBattleAction(sessionId, attackerId, targetId, action, baseDamage = 10) {
        ...
        const [attacker] = await db.select()
            .from(gameParticipants)
            .where(and(eq(gameParticipants.sessionId, sessionId), eq(gameParticipants.userId, attackerId)));
        const [target] = await db.select()
            .from(gameParticipants)
            .where(and(eq(gameParticipants.sessionId, sessionId), eq(gameParticipants.userId, targetId)));
        if (!attacker || !target) {
    ...
    async getPlayerStarterBurblemons(userId) {
        return db.select()
            ...
            .where(and(eq(playerBurblemons.userId, userId), eq(playerBurblemons.isStarter, true)));
    }
