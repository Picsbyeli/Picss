...
function fuzzyMatch(userAnswer, correctAnswer) {
    ...
    if (similarity >= 0.6) {
        ...
        const hasAllMainWords = correctWords.every(word => word.length <= 2 || userWords.some(userWord => userWord.includes(word) || word.includes(userWord)));
        if (hasAllMainWords) {
            return {
                isCorrect: true,
                confidence: similarity,
...
export async function checkAnswer(userAnswer, correctAnswer, question, hint, useAI = true) {
