...
export async function registerRoutes(app) {
    ...
    app.post('/api/check-answer', async (req, res) => {
        try {
            ...
            const answerCheckResult = await checkAnswer(answer, riddle.answer, riddle.question, riddle.hint || undefined, true // Enable AI checking
            );
            ...
            if (isCorrect && !alreadyViewedAnswer) {
                ...
                if (riddle.difficulty === 'easy')
                    ...
                    basePoints = 25;
                else if (riddle.difficulty === 'extreme')
                    basePoints = 40;
    ...
    app.get('/api/quiz/topics/:id/questions', async (req, res) => {
        try {
            ...
            const questions = await storage.getQuestionsByTopic(topicId, difficulty, limit ? Number(limit) : 10);
            res.json(questions);
        }
        catch (error) {
    ...
    app.post('/api/quiz/topics', async (req, res) => {
        try {
            ...
            const userId = req.user?.id || null; // Get user ID from session if available
    ...
    app.post('/api/quiz/validate-answer', async (req, res) => {
        try {
            ...
            const result = await storage.validateQuizAnswer(validatedData.questionId, validatedData.userAnswer);
            res.json(result);
        }
        catch (error) {
            if (error instanceof z.ZodError) {
    ...
    app.get('/api/dungeon/active', async (req, res) => {
        try {
            ...
            const dungeonRun = await storage.getActiveDungeonRun(playerId ? Number(playerId) : undefined);
            if (!dungeonRun) {
    ...
    app.post('/api/dungeon/:id/explore', async (req, res) => {
        try {
            ...
            const explorationItems = allItems.filter(item => item.itemType === 'healing' ||
    ...
    app.get('/api/leaderboard', async (req, res) => {
        try {
            ...
            const filteredUsers = users.filter(user => {
                ...
                if (user.email && (user.email.toLowerCase().includes('shit') ||
    ...
    app.post('/api/geography/submit', async (req, res) => {
        try {
            ...
            for (const region of regions) {
                ...
                const isCorrect = correctAnswers_arr.some(correct => userAnswer.includes(correct) || correct.includes(userAnswer));
                if (isCorrect) {
                    correctAnswers++;
                    currentStreak++;
                    ...
                    case 'submit-answer':
                        if (sessionId && userId) {
                            ...
                            if (allAnswered) {
                                ...
                                const allAnswersWithEffects = await Promise.all(answers.map(async (answer) => {
